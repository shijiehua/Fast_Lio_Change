...
void KD_TREE<PointType>::Build(PointVector point_cloud)
{
    ...
    if (point_cloud.size() == 0)
        return;
    STATIC_ROOT_NODE = new KD_TREE_NODE; // 是个结构体 表示节点
    InitTreeNode(STATIC_ROOT_NODE); //初始化当前节点的属性
    //建立kdtree，这里直接从其左子树开始构建  ,将所有点云都建在左子树中
    BuildTree(&STATIC_ROOT_NODE->left_son_ptr, 0, point_cloud.size() - 1, point_cloud);
    Update(STATIC_ROOT_NODE); // 里面跟新树中各个节点的属性，包含子节点连接父节点
    STATIC_ROOT_NODE->TreeSize = 0; // 为什么？
    // 将左子树直接替换根节点  为什么不直接从root开始创建树？
    Root_Node = STATIC_ROOT_NODE->left_son_ptr;
}
// K近邻搜索
template <typename PointType>
...
{   // 初始化自定义的大顶堆，容量为2*k_nearest
    ...
    q.clear();
    vector<float>().swap(Point_Distance); // 清空Point_Distance
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
    {
    ...
    {// 优先取出最近的点  //为社么这里好像是优先取大根堆中最远的点 ？ 是为了求平面方程更稳定吗
...
int KD_TREE<PointType>::Add_Points(PointVector &PointToAdd, bool downsample_on)
{
    ...
    for (int i = 0; i < PointToAdd.size(); i++)
    {
        ...
        {   // 计算该点所属的体素
            ...
            Search_by_range(Root_Node, Box_of_Point, Downsample_Storage); // 在树中搜索未删除的点，且要求点都在Box中，将点存于Downsample_Storage
            min_dist = calc_dist(PointToAdd[i], mid_point); // 打算加入的点与体素中心点的距离
            downsample_result = PointToAdd[i];  // 记录离体素中心点最近的点
            ...
            { // 遍历体素范围内的地图点
                ...
                {
                    min_dist = tmp_dist;
                    downsample_result = Downsample_Storage[index];  // 找到离体素中心点最近的点
                }
            }
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
            {
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result))
                {
                    if (Downsample_Storage.size() > 0)
                        Delete_by_range(&Root_Node, Box_of_Point, true, true); // 为什么要删除体素内的地图点 是为了体素降采样 
                        // 删除地图点，加入新的更好的点代表整个体素
                    Add_by_point(&Root_Node, downsample_result, true, Root_Node->division_axis); // 加入距离体素中心最近的点 到树中
                    tmp_counter++;
                }
            }
            else
...
{ // 递归的方法将树， 有点像快排  l表示点云的左索引  r表示点云右索引
    if (l > r)
        return;
    *root = new KD_TREE_NODE;
    InitTreeNode(*root); // 初始化一个根节点
    int mid = (l + r) >> 1;  // 位运算 ，这里是取中间值 属于整除2运算
    int div_axis = 0;
    int i;
    ...
    // Select the longest dimension as division axis  以值范围最大的维度为分割维度
    for (i = 0; i < 3; i++)
    ...
    {
    case 0:
        // point_cmp_x是个比较函数   对小于中间位置的值放在左边，大于中间的值放在右边，确保中间位置的值是中间值
        // 第 mid 个位置的元素会被放到它在完全排序后应该处于的位置。
    ...
    (*root)->point = Storage[mid]; // 将中间点设为该节点的值
    ...
    // 上面好像没有链接父节点
    Update((*root));  // 里面跟新树中各个节点的属性，包含子节点连接父节点
    return;
}
...
void KD_TREE<PointType>::Add_by_point(KD_TREE_NODE **root, PointType point, bool allow_rebuild, int father_axis)
{
    ...
    if (Rebuild_Ptr != nullptr && *Rebuild_Ptr == *root && (*root)->TreeSize < Multi_Thread_Rebuild_Point_Num)
        Rebuild_Ptr = nullptr;
    bool need_rebuild = allow_rebuild & Criterion_Check((*root)); // 检验后发现不平衡的话就需要重建该子树
    if (need_rebuild)
        Rebuild(root);
    if ((*root) != nullptr)
        (*root)->working_flag = false;
...
void KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist)
{
    if (root == nullptr || root->tree_deleted)
        return;
    float cur_dist = calc_box_dist(root, point);   // 计算点到 树点云的取值范围框的距离平方
    float max_dist_sqr = max_dist * max_dist;  // 这是无限大
...
void KD_TREE<PointType>::Search_by_range(KD_TREE_NODE *root, BoxPointType boxpoint, PointVector &Storage)
{
    if (root == nullptr)
        return;
    Push_Down(root); // 跟新该节点的子节点的删除的属性  里面没有递归  但这里进行了递归 
    if (boxpoint.vertex_max[0] <= root->node_range_x[0] || boxpoint.vertex_min[0] > root->node_range_x[1])
        return; // 该树不在体素范围内
    if (boxpoint.vertex_max[1] <= root->node_range_y[0] || boxpoint.vertex_min[1] > root->node_range_y[1])
        return; // 该树不在体素范围内
    ...
    if (boxpoint.vertex_min[0] <= root->node_range_x[0] && boxpoint.vertex_max[0] > root->node_range_x[1] && boxpoint.vertex_min[1] <= root->node_range_y[0] && boxpoint.vertex_max[1] > root->node_range_y[1] && boxpoint.vertex_min[2] <= root->node_range_z[0] && boxpoint.vertex_max[2] > root->node_range_z[1])
    {   // 该树包含在体素范围内
        flatten(root, Storage, NOT_RECORD); // 将整棵树的未删除的点存入 Storage中
        return;
    }
    if (boxpoint.vertex_min[0] <= root->point.x && boxpoint.vertex_max[0] > root->point.x && boxpoint.vertex_min[1] <= root->point.y && boxpoint.vertex_max[1] > root->point.y && boxpoint.vertex_min[2] <= root->point.z && boxpoint.vertex_max[2] > root->point.z)
    { // 该节点被包含在体素范围内 ， 如果该点未删除，则加入该点
        if (!root->point_deleted)
            Storage.push_back(root->point);  // 加入该点
    }
    if ((Rebuild_Ptr == nullptr) || root->left_son_ptr != *Rebuild_Ptr)
    { 
        Search_by_range(root->left_son_ptr, boxpoint, Storage);  //递归左子树  
    }
    else
    {
        pthread_mutex_lock(&search_flag_mutex);
    ...
    if ((Rebuild_Ptr == nullptr) || root->right_son_ptr != *Rebuild_Ptr)
    {
        Search_by_range(root->right_son_ptr, boxpoint, Storage); // 递归右子树
    }
    else
    {
        pthread_mutex_lock(&search_flag_mutex);
...
{  // 惰性删除策略 ，这些点不会立即从树中删除，而只是通过将属性deleted
    // 更新该节点与子节点  删除相关的属性，没有递归，所以该子节点的子节点没有跟新
    if (root == nullptr)
        return;
...
void KD_TREE<PointType>::Update(KD_TREE_NODE *root)
{
    ...
    float tmp_range_x[2] = {INFINITY, -INFINITY}; // 统计该节点为根节点的树 每个维度的 最大值和最小值
    float tmp_range_y[2] = {INFINITY, -INFINITY};
    float tmp_range_z[2] = {INFINITY, -INFINITY};
    // Update Tree Size
    if (left_son_ptr != nullptr && right_son_ptr != nullptr)
    {   // 由于该函数在递归中， 因此这里的统计有递归的含义
        root->TreeSize = left_son_ptr->TreeSize + right_son_ptr->TreeSize + 1;  // 计算以该节点为根节点的树的点数数
        // 计算以该节点为根节点的树的无效点数
    ...
    if (root == Root_Node && root->TreeSize > 3)
    {
        ...
        root->alpha_del = float(root->invalid_point_num) / root->TreeSize;  // 树中无效点占比
        root->alpha_bal = (tmp_bal >= 0.5 - EPSS) ? tmp_bal : 1 - tmp_bal; // 左右子树中大的子树 相对于 两个子树和 的比例
    }
    return;
}
...
void KD_TREE<PointType>::flatten(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type)
{
    ...
    flatten(root->left_son_ptr, Storage, storage_type);  // 递归的将有效点存入Storage中
