...
public:
    ...
    {
        PointType point;
        int division_axis;  // 节点的分割维度
        int TreeSize = 1;
        int invalid_point_num = 0;
        int down_del_num = 0;  // 降采样删除的节点个数和
        bool point_deleted = false;  // 该节点是否被删除
        bool tree_deleted = false;  //树是否被完全删除
        ...
        float radius_sq;   // 树的中值点与地图起点的距离平方
        ...
        float alpha_del;  // 以该节点为根节点的树中 无效点占比  ，无效点就是删除的点
        float alpha_bal;  // 左右子树中大的子树 相对于 两个子树和 的比例 
    };
    ...
    private:
        ...
        {   // 循环找父节点 ，与父节点比较，如果比父节点的距离大则将父节点下沉，保证父节点的距离比子节点大
